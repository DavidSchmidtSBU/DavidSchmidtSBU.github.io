...
public class PPMPicture
{
    ...
    //POSCondition: pixels contains a negative of the grayscale image
    {
        ...
        for (int i = 0; i < pixels.length; i+=3)
        {
        	...
        	int green = pixels[i+1];
        	int blue = pixels[i+2];
        	int negative = 255 - (pixels[i]);
        	int negative1 = 255 - (pixels[i+1]);
        	int negative2 = 255 - (pixels[i+2]);
        	pixels[i] = negative;
        	pixels[i+1] = negative1;
        	pixels[i+2] = negative2;
        }
    }//setToNegative
    ...
    //             
    {
        ...
        {
        	int art = pixels[i] + 255 - (pixels[i]);
        	int art1 = pixels[i+1] + 255 - (pixels[i+1]);
        	int art2 = pixels[i+2] + 255 - (pixels[i+2]);
        }
    }//myArtwork
